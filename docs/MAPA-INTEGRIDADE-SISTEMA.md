...
**Última atualização:** 2026-06-16 — **Sprint M39 Emissão Admin em Lote** (Blocos 1-8 entregues, 7 commits `bf3d658..fe9d9c1`; reviewers + smoke + push pendentes no Bloco 9). Substitui o caminho admin de `enviarTokensAdmin` single-target (@deprecated, mantido por COMPAT) por novo endpoint `POST /cooper-token/admin/emitir-lote` via helper `executarMassWrite`. Frontend `/dashboard/cooper-token/enviar` redesenhado em 2 etapas (seleção+confirmação) com filtro opcional por convênio + tabela editável + prévia COMPLETA antes do OTP. Tela nova `/dashboard/cooper-token/lotes-emitidos` com modal de estorno (confirmação dupla + motivo ≥10 chars + reversão atômica saldo+contábil+ledger ESTORNO). Schema delta aditivo: enum CooperTokenTipo += `BONIFICACAO_ADMIN` + `ESTORNO_BONIFICACAO_ADMIN` + conta `5.1.03 "Despesa de Bonificação CooperToken"` (DESPESA). 2 templates contábeis novos: `lancarEmissaoAdminLote` (D 5.1.03 / C 5.1.02) + `lancarEstornoEmissaoAdminLote` (D 5.1.02 / C 5.1.03). Bypass do `COOPER_TOKEN_EVENTS.EMITIDO` no caminho admin pra não disparar template errado da F1. **20/20 specs M39 verde** + **284/284 cooper-token completo verde** (zero regressão das 264 anteriores). Débito P2 catalogado `D-novo-EMISSAO-ADMIN-CONTABIL` (sprint contábil dedicada: corrige tipo conta 5.1.02 DESPESA→PASSIVO + template F2 errado + F3/F6/clube sem LancamentoCaixa — depende de D3 Modelo C preço custo×venda).

> Histórico: **2026-05-30 noite — Sprint D-novo-AN (RepasseProprietario) COMPLETO 100%** (5 commits `37f7af0..2f6fb29` em 1 sessão Code dia inteiro, AN.1→AN.4 + AN.3.1 fix bônus). Workflow PENDENTE→PAGO/CANCELADO com transação atômica vinculando despesas DESCONTO_NO_REPASSE. Cron BH.5 agora cria par Repasse+ARRENDAMENTO_USINA em $transaction. Telas admin (por usina + global) + portal proprietário com status real + sidebar item Wallet + card cruzado. Backfill executado: 3 históricos PENDENTE preservando 04/2026 trigger AN.3.1. Notificação proativa "Repasse pago" fire-and-forget. PDF mensal com status real removendo heurística fake. **36/36 specs verdes** + 3 smokes programáticos + smoke HTTP 4/4. D-novo-BM funcionalmente reparado inline AN.3.1 (TTL 1h→8h + interceptor allowlist). D-novo-BP catalogado (convergência /parceiro vs /dashboard, P3 sprint refator UX futuro).

> Histórico: **2026-05-30 — Sprint D-novo-BH (Despesas Operacionais Camada 2) COMPLETO 100%** (10 commits bb838ec..77eeb24 em 2 sessões Code, M37→M41 + 3 bugs/débitos bônus). Workflow proposta→aprovação→resolução com double-check universal + portal proprietário + flag visibilidade + helper líquido (envelope sobre `calcularRepasse` puro) + cron mensal aluguel automático ARRENDAMENTO_USINA idempotente. **55 specs Jest verdes + 24/24 smokes programáticos.** Bugs/débitos bônus resolvidos inline: D-novo-BL (Super Admin bypass), D-novo-BN (ChunkLoadError Turbopack stale). D-novo-BM implementado P0 BLOQUEADOR REMOÇÃO PRÉ-PROD (painel credenciais teste dev-only, 4 camadas de defesa). Lição arquitetural: build web SEMPRE seguido de pm2 restart imediato (complemento D-novo-AS).

> Histórico: **2026-05-18 noite tarde — Sub-Fase 1 Listas Concessionária COMPLETA + Sprint 8 / Bloco E Realocação Multi-Usina COMPLETO**.

## Matriz executiva (30/05/2026)

| Indicador | Antes | Depois | Evidência |
|---|---|---|---|
| Segurança multi-tenant | 🔴 | 🟢 BLINDADO | **Sprint Blindagem Multi-Tenant (D-novo-BR) COMPLETO 31/05** — 8 commits em 1 sessão. **68/68 IDORs do sistema corrigidos** em 6 fatias atômicas (F0+F1.1+F1.2+F1.3+F1.4+F1.5). **4 camadas de defesa em profundidade ATIVAS:** (1) fix manual ponto-a-ponto, (2) Guard sistêmico opt-in `@TenantResource` + `TenantOwnershipGuard` (F1.1+F1.2 anotou 15 endpoints), (3) Extension Prisma log-only `tenantLeakDetector` + ALS `runWithTenant`/`runAsPlatform` em 45 métodos cron/listener (F1.3), (4) Lint anti-reincidência baseline+ratchet `npm run lint:tenant` com 256 legados allowlist (F1.4). **164 specs IDOR+Guard verdes** + **6 smokes runtime cross-tenant** validando 91 cenários. EmailLog ganhou `cooperativaId` via ritual PM2 (M7). M8 email-monitor fallback ENV removido (vazava credencial IMAP entre tenants). Sprint estreou padrão **F2 Prisma Extension de INJEÇÃO automática como OPCIONAL** (Guard+lint+log já cobrem detecção pre-merge + runtime). Detalhe: `docs/sessoes/2026-05-31-sprint-blindagem-multi-tenant-completo.md`. |
...
| **Despesas Operacionais Camada 2 (D-novo-BH)** | 🔴 | 🟢 | **Sprint D-novo-BH COMPLETO 100% em 30/05** — 7 fatias BH.1→BH.5 (10 commits `bb838ec..77eeb24`). Workflow proposta→aprovação double-check (TODOS perfis criam PROPOSTA, self-approval guard backend)→resolução com tratamento REEMBOLSO/DESCONTO_NO_REPASSE/ASSUMIDO. Portal proprietário com flag visibilidade default-false (opt-in admin). Helper `calcularRepasseLiquido` envelope sobre `calcularRepasse` puro (intacto) — 7 consumidores migrados, líquido nunca negativo. Cron mensal `@Cron('0 3 1 * *', tz São Paulo)` cria ARRENDAMENTO_USINA APROVADA+RESOLVIDA+ASSUMIDO+PARCEIRO idempotente. Endpoint manual trigger DEV-only `isAmbienteReal()=false`. **55 specs Jest verdes + 24/24 smokes programáticos.** Resolve D-novo-BL (Super Admin bypass tenant) inline. |
| **Painel Credenciais Teste DEV (D-novo-BM)** | — | 🟢 / 🚨 | **IMPLEMENTADO P0 BLOQUEADOR REMOÇÃO PRÉ-PROD** em 30/05 (`1cdb9cb`). `POST /auth/dev/impersonate` gera JWT temporário TTL 1h sem expor senha. Defesa em 4 camadas: `isAmbienteReal()` runtime + `@Roles(SUPER_ADMIN)` + `@AuditLog` + JWT TTL curto. Painel `/dashboard/dev/credenciais-teste` com banner vermelho gigante "DEV ONLY". Item sidebar condicional via probe. **OBRIGAÇÃO** remoção em 9 passos catalogada em `docs/debitos-tecnicos.md` quando primeiro parceiro real entrar em produção. |
| **Cache Turbopack pós-rebuild (D-novo-BN)** | 🔴 | 🟢 | **RESOLVIDO em 30/05** (`03f49fc`, 15min). ChunkLoadError em `/dashboard/usinas/[id]/despesas` causado por chunk Turbopack stale após rebuild incremental durante runtime PM2. Fix conservador: `pm2 stop frontend + rm -rf web/.next + npm run build + pm2 start`. Lição catalogada como complemento D-novo-AS: **toda execução de `npm run build` em `web/` OBRIGATORIAMENTE seguida de `pm2 restart cooperebr-frontend` IMEDIATO.** Aplicado 8× nos sprints BH+AN sem regressão. |
| **RepasseProprietario (D-novo-AN)** | 🔴 | 🟢 | **Sprint D-novo-AN COMPLETO 100% em 30/05 noite** — 5 fatias AN.1→AN.4 (5 commits `37f7af0..2f6fb29`). Model `RepasseProprietario` com `@@unique([usinaId, periodoInicio, periodoFim])` idempotência forte + back-ref `ContaAPagar.repasseAbatido`. Workflow PENDENTE→PAGO/CANCELADO via transação atômica (`$transaction([repasse.update, contaAPagar.updateMany WHERE tratamento=DESCONTO_NO_REPASSE AND statusAprovacao=APROVADA AND statusResolucao=PENDENTE AND dataOcorrencia IN período])`). Cron BH.5 agora cria par Repasse+ARRENDAMENTO_USINA em $transaction. 6 endpoints REST + 2 telas admin + portal proprietário com fallback REAL/PREVISTO_FALLBACK + sidebar Wallet + card cruzado. Notificação proativa "Repasse pago" fire-and-forget (email + WA whitelist LGPD com Caminho A/B). PDF mensal com status real. **36/36 specs Jest verdes** + 3 smokes programáticos. Backfill executado: 4 PENDENTE no banco (02/03/04/05 2026). Integração BH↔AN: despesas DESCONTO_NO_REPASSE pendentes do período são automaticamente abatidas + resolvidas quando admin marca repasse PAGO. |
| **Painel Credenciais Teste (D-novo-BM) reparo funcional** | 🟡 | 🟢 | **Reparo AN.3.1 30/05** (`3a8a90e`). Painel voltava pro `/login` em uso real porque token impersonate TTL 1h expirado causava `useContexto.GET /auth/me` 401 → interceptor global redirect ANTES do `.catch()` da página. Fix duplo: (A) backend TTL 1h→8h `auth.service.ts.assinarTokenImpersonate` (dev-only, gating mantido) + (B) frontend allowlist self-recovery em `lib/api.ts` (rotas `/dashboard/dev/credenciais-teste` e `/selecionar-contexto` não redirecionam em 401) + UI inline `sessaoExpirada` com botões "Ir pra /login" / "Tentar de novo". **Status BLOQUEADOR REMOÇÃO PRÉ-PROD mantido** — reparo só ajustou usabilidade DEV. |
...
- **Cadastro Usina** 🟡 Parcial → 🟢 OK. Schema expandido (11 campos + 2 enums FormaAquisicao/FormaPagamentoDono), saneamentos AMAGES + Exfishes (CTR-000134 estado dirty), Cooperebr2 cadastrada (Linhares 2, CUSD EDP-ES-04123/2025), apelidoInterno cooperebr1/cooperebr2, UI cadastro com campos condicionais FIXO/PERCENTUAL/HIBRIDO (Mini-Bloco H'.9 17/05). **`classeGd`/`RegrasFioB`/guards intencionalmente fora** (Sprint Módulo Classificação GD separado pós-dossiê judicial). Commits `2024b13` + `15db027` + `9dada58`.
...
| 7 | Contabilidade & Financeiro | ESTRUTURA COMPLETA — aguarda gate de validação fiscal | 95% técnico + 0% validação | **Sprint Contabilidade Tributária Segregada CT.1-CT.6 100% (31/05/2026)** — motor multi-regime + 4 DREs + 3 PDFs + 4 telas + estorno repasse com ciclo contábil. Aguarda Luciano + orquestrador validamr alíquotas/presunção + classificação + 10 contas seed antes de produção fiscal real (D-novo-CT-VALIDACAO-FISCAL P0). |
...
| Etapa | Status | Arquivo | Gap |
|-------|--------|---------|-----|
| `LancamentoCaixa` CRUD | FUNCIONAL + CLASSIFICAÇÃO AUTOMÁTICA | `financeiro/lancamentos.service.ts` + hooks CT.3 (Cobranca/ContaAPagar/Repasse) | ✅ Sprint CT.3 (31/05/2026) wirou hooks fire-and-forget classificando `naturezaAto` na criação. `@@unique([origemTipo, origemId])` garante idempotência. Campo `naturezaAto` mantido String com enum `NaturezaCooperativa` usado via TypeScript (promoção formal pra coluna enum deferida pra migration SQL com USING cast — não-bloqueante). |
| Plano de contas CRUD | FUNCIONAL + SEGREGAÇÃO COOPERATIVA | `financeiro/plano-contas.service.ts` + tela read-only `dashboard/contabilidade/plano-contas` | ✅ Sprint CT.1 adicionou `naturezaContabil` + `naturezaCooperativa` + `fundamentoLegal` (nullable pra preservar 24 contas seed legadas). 12 contas seed CT.1 já classificadas. CRUD continua em `/dashboard/configuracoes/financeiro`. |
...
| DRE (Demonstração Resultado) | FUNCIONAL (4 visões) | `contabilidade-tributaria/dre.service.ts` + `dashboard/contabilidade/dre` | ✅ Sprint CT.5 (31/05/2026) entregou 4 DREs (Geral/Próprio/Auxiliar/Não-Coop) com terminologia NBC ITG 2004 (ingressos/dispêndios). Lê snapshot fechado OU preview on-the-fly. Badge GATE VALIDAÇÃO FISCAL quando `validadoContador=false`. Endpoint `GET /contabilidade-tributaria/dre/:visao?ano&mes`. |
...
| **Segregação Ato Cooperativo Próprio × Auxiliar × Não Cooperativo** | **FUNCIONAL** (classificação automática hooks CT.3) | `contabilidade-tributaria/regimes/cooperativo.regime.ts` + hooks CT.3 | ✅ Sprint CT.1+CT.2+CT.3 (31/05/2026). Regra: COBRANCA cooperado COM_UC/SEM_UC/GERADOR→PROPRIO; USUARIO_CARREGADOR/CARREGADOR_VEICULAR→NAO_COOP. REPASSE Usina.formaAquisicao ALUGUEL→NAO_COOP; CESSAO/PROPRIA→PROPRIO. CONTA_A_PAGAR→PROPRIO. CONVENIO→AUXILIAR. Determinístico. Outros regimes (CONSORCIO/ASSOCIACAO/CONDOMINIO) → `NotImplementedException` explícito (P0-1). |
| **Apuração tributária por natureza** (PIS/COFINS/IRPJ/CSLL/ICMS isentos sobre ato próprio) | **FUNCIONAL** (motor mensal + snapshot imutável) | `contabilidade-tributaria/apuracao.service.ts` + `dashboard/contabilidade/apuracao` | ✅ Sprint CT.4 (31/05/2026). `apurarMes` (preview) + `fecharApuracao` (snapshot `ApuracaoMensalSegregada` imutável, race-guard P2002) + `validarApuracao` (gate de validação fiscal) + `reabrirApuracao` (SA-only motivo ≥10) + bloqueio retroativo CT.3 (mês FECHADA bloqueia hook). Tributos calculados sobre não-coop; sobras isentas (RIR Art. 182); flag `isencaoPisCofinsAtiva` controla PIS/COFINS próprio (P0-4 STF Tema 536). `ConfiguracaoTributaria` por cooperativa (zero hardcoded). |
| **Demonstrativos fiscais defensáveis** (Memorial de Cálculo, Não-Lucratividade, Repasses) | **FUNCIONAL** (3 PDFs com watermark) | `contabilidade-tributaria/relatorios-ct.service.ts` | ✅ Sprint CT.6 (31/05/2026). Puppeteer HTML→PDF reusado de motor-proposta. Watermark "PENDENTE VALIDAÇÃO FISCAL" + banner condicional. Demonstrativo Não-Lucratividade cita Art. 79+STF Tema 536+RIR Art. 182. Memorial Cálculo Fiscal cita 4 leis (5.764/71+9.249/95+9.718/98+7.689/88) passo-a-passo. Demonstrativo Repasses agrupa por `Usina.formaAquisicao` (ALUGUEL=NÃO-COOP × CESSÃO/PRÓPRIA=PRÓPRIO). |
| **Estorno repasse + visibilidade ciclo contábil** | **FUNCIONAL** (backend) — UI Dialog refator PUX.4 | `repasses-proprietario.service.ts.estornarRepasse` + `obterCicloRepasse` | ✅ Fatia 31/05/2026 (`93f38da`). Estorno atômico: status→PENDENTE + deleta LancamentoCaixa (libera `@@unique`) + desvincula despesas. Gate: apuração FECHADA bloqueia (mensagem clara "reabra primeiro SA"). Visibilidade ciclo: `{ repasse, lancamentoGerado, despesasAbatidas[] }`. **Frontend usa Dialog Tipo C — refator PUX.4 vira inline expansível.** |
...
**% Pronto:** 95% técnico (financeiro genérico 50% + tributário cooperativo CT.1→CT.6 100% estrutura) + 0% validação fiscal interna (Luciano + orquestrador) (gate produção). **Gap cooperativo:** ✅ 100% técnico — segregação + apuração + DREs + PDFs + estorno todos prontos em commits `5ada766..93f38da` (31/05/2026). **Bloqueador único pra produção fiscal real:** **D-novo-CT-VALIDACAO-FISCAL P0** — validação interna (Luciano + orquestrador) valida defaults `ConfiguracaoTributaria` + classificação repasse + 10 contas seed + 10 lançamentos amostrais + flag `isencaoPisCofinsAtiva` (STF Tema 536). Snapshot só vira oficial após `PUT /apuracao/:id/validar`. **Pendências não-bloqueantes restantes:** conciliação bancária (Sprint 7 genérico); SPED/NF3e/eSocial (Sprint D-55 ~40-60h separado); Sprint Polimento UX (refator frontend 4 telas CT.6 de Dialog→inline expansível, ~25-37h Code).
