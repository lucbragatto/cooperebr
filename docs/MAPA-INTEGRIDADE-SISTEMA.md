...
|---|---|---|---|
| Segurança multi-tenant | 🔴 | 🟢 | **Sprint Blindagem Multi-Tenant Fase 0 (D-novo-BR F0) COMPLETO 31/05** — fix manual em lote de 26 IDORs (19 Onda A + 7 críticos Onda B). Auditorias expandidas (Dynamic Workflow Onda A 25 sub-agentes + Onda B 45 sub-agentes) revelaram 50 IDORs adicionais → totalizando 68 IDORs sistêmicos no backend. **44 corrigidos** (18 D-novo-BQ + 26 D-novo-BR F0), 24 restantes deferidos pra D-novo-BR Fase 3. **111 specs IDOR verdes** (56 BQ + 55 BR F0) + **58 cenários runtime cross-tenant** (35 BQ + 23 BR F0) validados em 4 smokes programáticos. 4 padrões consolidados: posse direta, posse via-relação, body→JWT (helper resolverTenant), global-only-SA. Decisão arquitetural híbrida em 5 fases catalogada em `docs/arquitetura/blindagem-multi-tenant-sistemica.md` (F0 ✅ / F1 AsyncLocalStorage / F2 Prisma Client Extension / F3 residuais / F4 testes regressão). **Anterior:** D-novo-BQ Sprint Segurança IDOR (18 IDORs núcleo 30/05) + D-48 + Fase 2A-2E (35 endpoints). |
