...
| 7 | Contabilidade & Financeiro | ESTRUTURA COMPLETA — aguarda gate Walter | 95% técnico + 0% validação | **Sprint Contabilidade Tributária Segregada CT.1-CT.6 100% (31/05/2026)** — motor multi-regime + 4 DREs + 3 PDFs + 4 telas + estorno repasse com ciclo contábil. Aguarda Walter validar alíquotas/presunção + classificação + 10 contas seed antes de produção fiscal real (D-novo-CT-GATE-WALTER P0). |
...
| DRE (Demonstração Resultado) | FUNCIONAL (4 visões) | `contabilidade-tributaria/dre.service.ts` + `dashboard/contabilidade/dre` | ✅ Sprint CT.5 (31/05/2026) entregou 4 DREs (Geral/Próprio/Auxiliar/Não-Coop) com terminologia NBC ITG 2004 (ingressos/dispêndios). Lê snapshot fechado OU preview on-the-fly. Badge GATE WALTER quando `validadoContador=false`. Endpoint `GET /contabilidade-tributaria/dre/:visao?ano&mes`. |
...
| **Apuração tributária por natureza** (PIS/COFINS/IRPJ/CSLL/ICMS isentos sobre ato próprio) | **FUNCIONAL** (motor mensal + snapshot imutável) | `contabilidade-tributaria/apuracao.service.ts` + `dashboard/contabilidade/apuracao` | ✅ Sprint CT.4 (31/05/2026). `apurarMes` (preview) + `fecharApuracao` (snapshot `ApuracaoMensalSegregada` imutável, race-guard P2002) + `validarApuracao` (gate Walter) + `reabrirApuracao` (SA-only motivo ≥10) + bloqueio retroativo CT.3 (mês FECHADA bloqueia hook). Tributos calculados sobre não-coop; sobras isentas (RIR Art. 182); flag `isencaoPisCofinsAtiva` controla PIS/COFINS próprio (P0-4 STF Tema 536). `ConfiguracaoTributaria` por cooperativa (zero hardcoded). |
| **Demonstrativos fiscais defensáveis** (Memorial de Cálculo, Não-Lucratividade, Repasses) | **FUNCIONAL** (3 PDFs com watermark) | `contabilidade-tributaria/relatorios-ct.service.ts` | ✅ Sprint CT.6 (31/05/2026). Puppeteer HTML→PDF reusado de motor-proposta. Watermark "PENDENTE VALIDAÇÃO CONTADOR" + banner condicional. Demonstrativo Não-Lucratividade cita Art. 79+STF Tema 536+RIR Art. 182. Memorial Cálculo Fiscal cita 4 leis (5.764/71+9.249/95+9.718/98+7.689/88) passo-a-passo. Demonstrativo Repasses agrupa por `Usina.formaAquisicao` (ALUGUEL=NÃO-COOP × CESSÃO/PRÓPRIA=PRÓPRIO). |
...
**% Pronto:** 95% técnico (financeiro genérico 50% + tributário cooperativo CT.1→CT.6 100% estrutura) + 0% validação Walter (gate produção). **Gap cooperativo:** ✅ 100% técnico — segregação + apuração + DREs + PDFs + estorno todos prontos em commits `5ada766..93f38da` (31/05/2026). **Bloqueador único pra produção fiscal real:** **D-novo-CT-GATE-WALTER P0** — Walter (contador) valida defaults `ConfiguracaoTributaria` + classificação repasse + 10 contas seed + 10 lançamentos amostrais + flag `isencaoPisCofinsAtiva` (STF Tema 536). Snapshot só vira oficial após `PUT /apuracao/:id/validar`. **Pendências não-bloqueantes restantes:** conciliação bancária (Sprint 7 genérico); SPED/NF3e/eSocial (Sprint D-55 ~40-60h separado); Sprint Polimento UX (refator frontend 4 telas CT.6 de Dialog→inline expansível, ~25-37h Code).
