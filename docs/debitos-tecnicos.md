...
- **🔴 P1 `D-novo-SCHEMA-DRIFT-EMAILALIASCAMPANHA` (ELEVADO 2026-07-20 — bloqueia próximo db push + migration ausente do webhook_events)** — coluna `contratos_convenio.emailAliasCampanha` existe no banco Supabase (1 valor não-nulo) mas foi removida do schema Prisma há tempos (zero referências em `backend/src`). **Consequências ativas:** (a) `prisma db push` bloqueia com aviso de data-loss → próxima alteração de schema fica travada; (b) a tabela `webhook_events` (criada nesta corretiva via SQL raw one-off) NÃO tem migration correspondente — `prisma migrate reset` OU recriação do banco a partir de migrations NÃO recria a tabela + índice UNIQUE. Idempotência do webhook depende de infra que não está no versionamento canônico. **Fix:** (1) auditoria (script one-off que mostra o único valor + FK/refs no banco) → decisão sobre o valor; (2) resolver o drift (`DROP COLUMN` manual OU `db push --accept-data-loss` autorizado); (3) gerar migration formal pra `webhook_events` (`prisma migrate dev --name add_webhook_events`) pra selar a fonte da verdade schema↔banco. Descoberto na Corretiva Asaas Webhook 2026-07-20 quando `db push` bloqueou aplicação do model `WebhookEvent`.
