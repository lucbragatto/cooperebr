...
**Última atualização:** 2026-07-22 (tarde pré-Tarefa 4) — **AgentsModule (IAG) órfão preservado em branch WIP**. **1 NOVO catalogado:**

- **P3 `D-novo-AGENTS-SUBSISTEMA-ORFAO`** — 19 arquivos em `backend/src/agents/` (2.573 linhas), sprint iniciado 11/06/2026 e pausado, permaneceram untracked no `main` desde então. **NÃO importado em `app.module.ts`** (código morto em runtime, zero risco de segurança), MAS dentro de `src/` o `tsc` compila em todo build — erro de tipo futuro quebraria o build sem causa aparente. Também estava a um `git clean` de sumir e NÃO constava na frase de retomada, arriscando limpeza sem contexto na próxima sessão. **Preservado em `wip/agents-subsistema-1106`** (padrão da branch `feature/mascara-email-convenio` de 14/07): commit `46255c2` na branch WIP, `main` limpo (arquivos removidos automaticamente no `git checkout main`). **Conteúdo** (README.md preservado no commit): `agents.module.ts` + `agents.service.ts` (infra base) + `common/policy/` (PolicyEngine com níveis L0-L4: L0 leitura, L1 simulação, L2 baixo risco, L3 financeiro/regulatório com aprovação humana, L4 bloqueado) + `common/tools/` (ToolRegistry) + `common/types/` (policy.types) + 3 subsistemas em estágio inicial: `sentinela/` (prio A), `repasses-despesas/` (prio B), `cobranca/` (prio E). **Princípios do módulo:** dependência unidirecional (agents/ usa o core; core NUNCA depende de agents/), zero autonomia sem governança (toda ação passa pelo PolicyEngine), rastreabilidade nativa via `@AuditLog` + `AuditLog` table + `EventEmitter2`, TDD com cobertura mínima 80% nas partes críticas. **Retomada futura:** `git checkout wip/agents-subsistema-1106` + continuar. **Não bloqueia Tarefa 4.**
...
**Última atualização:** 2026-07-21 (tarde) — **Diagnóstico de alarme falso do teste de login** (DHCP mudou IP, `NEXT_PUBLIC_API_URL` congelado no build). **2 NOVOS catalogados:**

- **P2 `D-novo-API-URL-IP-CONGELADO`** — `web/.env.local:NEXT_PUBLIC_API_URL=http://192.168.3.<IP>:3000` é **compilado no build do Next.js** (`NEXT_PUBLIC_*` é substituído em tempo de build, não runtime). Toda mudança de DHCP do host quebra o login inteiro sem ninguém associar a causa: `POST /auth/login` dá `ERR_CONNECTION_TIMED_OUT` no browser, e o UI mostra "Identificador ou senha inválidos" (ver `D-novo-LOGIN-ERRO-ENGANOSO` abaixo) → diagnóstico errado garantido. **Aconteceu 21/07 tarde**: DHCP mudou de `192.168.3.88` pra `192.168.3.11`, teste de login humano do algorithms HS256 nem chegou no backend, pareceu erro do fix. **Fix estrutural proposto:** frontend chamar caminho relativo `/api/*` com `rewrite` do Next apontando pro backend — navegador fala com quem serviu a página, IP deixa de importar (funciona por qualquer host: `localhost`, IP LAN, domínio). **Alternativa paliativa:** reservar IP fixo no roteador pro host de dev (não resolve o problema estrutural, só reduz probabilidade). **Origem:** sessão 2026-07-21 corretiva IDOR (Task 3 spec HS256 quase deu falso-positivo por causa disso).

- **P2 `D-novo-LOGIN-ERRO-ENGANOSO`** (UX) — falha de REDE no `POST /auth/login` (connect timeout, DNS falha, backend down, CORS bloqueia) é exibida na tela de login como **"Identificador ou senha inválidos"** — mesma mensagem de credenciais erradas. Diagnóstico errado garantido pro time e pro cooperado real (que vai achar que digitou senha errada e resetar sem necessidade, ou desistir). **Fix:** diferenciar no `catch` do form de login: `err.response?.status === 401` → "Identificador ou senha inválidos"; sem `response` (network error) OU `status >= 500` → mensagem específica ("Não conseguimos falar com o servidor agora. Tente novamente ou avise suporte."). Idealmente, telemetria pro Sentry/console também. **Origem:** sessão 2026-07-21 (mesmo incidente do `D-novo-API-URL-IP-CONGELADO` acima).

---

**Última atualização:** 2026-07-21 (tarde) — **Corretiva IDOR/multi-tenant sessão dedicada — Ondas 1+2+3 + revisão pelos 2 subagents (cooperebr-multitenant-reviewer + cooperebr-financeiro-token-reviewer)**. Fechados 14 furos + 3 fixes pós-revisão (P1×2 do padrão fail-CLOSED em documentos+contratos update, P2×1 findDisponiveis vazando usinas cross-tenant). **7 NOVOS catalogados dos revisores:**

- **P1 `D-novo-PIX-EXCEDENTE-SUPERADMIN-ORFAO`** (financeiro-token reviewer, `pix-excedente.service.ts:176-178`) — SUPER_ADMIN sem `body.cooperativaId` cria `TransferenciaPix` com `cooperativaId=null` no banco. Registro órfão sem tenant: invisível pra queries tenantizadas mas acessível em agregações globais. Quando flag PIX real ligar, se guarda linha 146 for bypassada por bug, PIX real pode ser enviado sem registro de tenant. **Fix:** se `dto.cooperativaId=null` E `dto.cooperadoId` fornecido, executar lookup pra descobrir cooperativaId real do cooperado e populá-la no dto antes de persistir. Fora do escopo desta corretiva IDOR.
- **P1 `D-novo-PIX-EXCEDENTE-IDEMPOTENCIA`** (financeiro-token reviewer, `pix-excedente.service.ts:176`) — sem guard de idempotência: double-click ou retry HTTP em conexão lenta cria 2 `TransferenciaPix` e dispara 2 PIX reais quando flag estiver ativa. **Fix:** unique constraint em `TransferenciaPix(cooperadoId, mesReferencia)` OU `referenciaId` único no DTO com guard `findFirst` antes do create. Padrão consolidado da Corretiva Asaas Webhook FASE 2 (`WebhookEvent @@unique`).
- **P2 `D-novo-PIX-EXCEDENTE-SUBTOTAIS-IMPOSTO`** (financeiro-token reviewer, `pix-excedente.service.ts:204-206`) — response expõe `impostos.IR`, `impostos.PIS`, `impostos.COFINS` rearredondados individualmente das vars brutas, enquanto `impostos.total` usa `transferencia.valorImpostos` do banco (soma dos brutos arredondada). Soma dos 3 subtotais pode diferir em 1 centavo do total gravado. Não é inconsistência contábil (dado persistido é consistente), mas response confunde. **Fix:** expor IR/PIS/COFINS proporcional ao gravado OU documentar que são aproximações indicativas.
- **P3 `D-novo-ASAAS-LOG-PIXCHAVE-VAZAMENTO`** (financeiro-token reviewer, `pix-excedente.service.ts:170`) — `err.response?.data?.errors || err.message` no log de erro do Asaas pode incluir `pixAddressKey` se o gateway espelhar o request no response de erro. **Fix:** sanitizar removendo `pixAddressKey` antes de logar OU logar somente `err.response?.data?.errors` (já mais seguro que `err.response?.data`).
- **P3 `D-novo-EMAILMONITOR-IDENTIFICARPOROCR`** (multitenant reviewer, `email-monitor.service.ts:191`) — método `identificarPorOcr` não foi auditado nesta corretiva (não foi tocado). Se retornar cooperado sem filtro por tenant, o gate novo do `uploadConcessionaria` no service pega o cross-tenant (defesa em profundidade), mas idealmente `identificarPorOcr` também deveria filtrar. **Fix:** auditar + garantir filtro por cooperativaId em qualquer path de identificação.
- **P3 `D-novo-CLUBE-VANTAGENS-QCOOPID-PADRAO`** (multitenant reviewer, `clube-vantagens.controller.ts:16,26`) — padrão pré-existente `req.user?.cooperativaId || qCoopId` em `getConfig` e `upsertConfig` não discrimina perfil: se um ADMIN sem cooperativaId no token existir (além do COOPERADO já documentado), ganha bypass silencioso via query param. Fix: migrar pro `resolveTenantIdFromReq`. Não é furo novo desta corretiva.
- **P3 `D-novo-TESTS-DRIFT-DOCUMENTOCOOPERADO`** (multitenant reviewer) — ausência de test de drift para `DocumentoCooperado` (usa `via: 'cooperado.cooperativaId'` em `carregarComPosse` do service). Outros models com padrão via: `CooperToken`, `Indicacao` — auditar. **Fix:** replicar padrão do test drift NULL do `idor-onda2.integration.spec.ts` (Item 8a) pra os outros models.

**Ampliações desta sessão:**
- `D-novo-UC-TENANT-DRIFT` (Onda 1 colateral): 19 UCs com `Uc.cooperativaId=NULL` + 2 com drift (coluna aponta tenant divergente do dono real via cooperado). Motivo real de usar `via: 'cooperado.cooperativaId'` em `@TenantResource` do item 8a. **Sem fix técnico** — decisão semântica: aceitar drift, usar `via` como fonte de verdade. Auditoria de dados fica pra sessão de higiene do banco.
- `D-novo-JWT-TTL-LONGO` (Onda 3 catálogo): TTL 7d largo, sem refresh token infra. Reduzir agora quebra UX. `algorithms:['HS256']` (Onda 3 item 13) mata o vetor crítico (algorithm confusion). Fica pra sessão dedicada de auth refresh.
- `D-novo-OCR-PUBLICO-TOKEN-SESSAO` (Onda 3 catálogo): item 11 fix atual apenas aperta throttle 5/min. OCR queima Claude API — throttle é mitigação, não solução. Débito: exigir token de sessão de cadastro no `/publico/processar-fatura-ocr`.

---

**Última atualização:** 2026-07-21 — **Fechamento ROTA B (Achado 3 rotação secret) + fix runbook clipboard**. **1 NOVO catalogado:**

- **P3 `D-novo-RUNBOOK-CLIPBOARD-CONTAMINACAO`** — o Bloco 5 do runbook `docs/seguranca/restart-coordenado-achado-3-4-8.md` usava `$novo = Get-Clipboard` pra recuperar o secret rotacionado entre os passos operacionais. **Problema:** qualquer Ctrl+C acidental do humano (seleção no VS Code, copiar de terminal, clipboard-monitor) sobrescreve o clipboard silenciosamente. Em 21/07 custou **20min de falso-negativo** — Smoke 3 retornava 401 mesmo com rotação correta aplicada porque `Get-Clipboard` retornava array de 10 linhas (1704 bytes UTF-8) contendo texto PT-BR residual da conversa (`—` U+2014, `ç`, `õ`); cada request enviava lixo textual como header `x-whatsapp-secret`. Investigação lateral (checagem BOM, `pm2 env`, análise `dotenv/ConfigModule`, `pm2 delete + start`, exame do `pm2-start.js`) apontava pra suposto cache de env do PM2 daemon. **Fix aplicado no mesmo commit desta catalogação**: Bloco 5.3.2 (b) do runbook trocou `Get-Clipboard` por leitura direta do `.env` (fonte-de-verdade imutável): `$novo = (Select-String -Path 'backend/.env' -Pattern '^WHATSAPP_WEBHOOK_SECRET=(.+)$' | Select-Object -First 1).Matches.Groups[1].Value`. **Sem tarefa restante** neste débito — só nota derivada: runbooks futuros que rotacionem secrets NÃO devem depender de clipboard vivo entre passos (padrão a propagar). **Origem:** sessão 2026-07-21 manhã, execução do Bloco 3 do runbook Achado 3 (última frente aberta da corretiva Mensageria WA — 9/9 fechada).

**Última atualização:** 2026-07-20 — **Corretiva WA Zumbi + Log Chaves** (bot mudo desde 17/07 destravado). **2 NOVOS catalogados:**

- **P2 `D-novo-WA-ZUMBI-PORTA-3002`** (AMPLIADO 2026-07-21 — cobre daemon inteiro, não só :3002) — `pm2 stop/restart` em `cooperebr-whatsapp` deixou processo órfão (PID 16048 iniciado 17/07 14:54) segurando `:3002` em `0.0.0.0` + IPv6 enquanto o PM2 subia nova instância em `127.0.0.1:3002`. Duas instâncias competindo pelo mesmo `auth_info` Baileys → Meta rejeita ambas com code 405 (Method Not Allowed) → `/status="failed"` **mesmo com corretiva ACL aplicada e `fetchLatestBaileysVersion` funcionando**. `pm2 restart` NÃO resolve — só `pm2 delete cooperebr-whatsapp` + `Stop-Process -Id <pid-órfão> -Force` + start limpo pelo ecosystem. Foi a raiz do "status failed mascarado" que fez o bot mudo desde 17/07 parecer problema de sessão Meta quando na verdade era conflito local. **Fix:** (a) guard de startup no `whatsapp-service/index.mjs` que aborta com mensagem clara se `:3002` já ocupado por processo não-controlado por PM2; (b) investigar por que `pm2 stop` deixa órfão (SIGKILL vs SIGTERM handling? Windows-specific? child-process não desanexado?); (c) documentar procedimento de cleanup no CLAUDE.md (seção PM2) junto ao regenerate Prisma. **Origem:** sessão 2026-07-20 corretiva WA — bot mudo desde 17/07 diagnosticado como órfão + hardcode de versão vencido, **ambos necessários** pra fix (só commit `82c9ebc` não bastava; só cleanup também não).

  **AMPLIAÇÃO 2026-07-21 — daemon PM2 inteiro pode ficar dessincronizado:** durante corretiva IDOR Onda 1, um respawn do daemon PM2 (causado por EPERM em `rpc.sock` no Windows) deixou os 3 processos (`backend`+`frontend`+`whatsapp`) VIVOS mas ÓRFÃOS da gestão do PM2 — `pm2 list` mostrava `errored`/`stopped` mesmo com as portas 3000/3001/3002 respondendo HTTP OK, e `pm2 restart` falhava (Namespace not found). O `pm2 kill` mata o daemon (não os processos), então precisa `Get-NetTCPConnection -LocalPort <p>` pra derivar PIDs + `Stop-Process -Id <pid> -Force` explícito nos 3 antes de `pm2 start ecosystem`. **Cascade descoberto:** ao matar wa-service abruptamente durante o reparo, a sessão Baileys se PERDE do lado do Meta (mesmo com `auth_info` intacto no disco) — `/status` volta com `awaiting_qr` e é preciso re-parear pelo celular. **Fixes adicionais:** (d) runbook oficial `docs/seguranca/pm2-daemon-repair.md` cobrindo a sequência completa (portas → kill daemon → force PIDs → confirmar vazio → ecosystem start); (e) investigar por que o daemon PM2 perde `rpc.sock` (perm ACL da corretiva 20/07 do repo? Windows Service Control interference?); (f) `whatsapp-service/index.mjs` deveria emitir SIGTERM graceful pro Baileys em `process.on('SIGTERM')` — desconexão limpa evita o cascade `awaiting_qr`. **Origem da ampliação:** sessão 2026-07-21 corretiva IDOR Onda 1 (pós-reparo PM2, sessão WA perdida no cascade).

- **P2 `D-novo-WA-LOG-CHAVES-SESSAO`** — `logs/wa-out.log` dumpa material criptográfico Signal Protocol da sessão Baileys (`privKey`, `rootKey`, `remoteIdentityKey`, `baseKey` do `pendingPreKey` com `signedKeyId`) em texto claro via pino verbose. Diretório `logs/` é gitignored (não vaza no repo) **mas** qualquer leak alternativo (backup automatizado, screen share, agente com acesso ao FS, tarball de suporte, sync na nuvem) vaza a **identidade completa da sessão WhatsApp** — atacante pode impersonar o número. **Fix:** (a) subir logger pino level de `trace`/`debug` pra `info` no `whatsapp-service/index.mjs`; (b) localizar callsite que dumpa objeto de estado inteiro (`logger.trace(state)` ou similar no bootstrap Baileys) e reduzir pra metadata sem chaves; (c) auditar outros callsites de logger pra garantir que não expõem `state.keys`, `state.creds.noiseKey`, `state.creds.signedIdentityKey`. **Origem:** sessão 2026-07-20 corretiva WA (observado nos logs durante restart de diagnóstico).

**Última atualização:** 2026-07-20 — **Corretiva Asaas Webhook FASE 2** (WebhookEvent insert-first idempotência + efeitos essenciais atômicos + fire-and-forget → awaited síncrono). **6 NOVOS catalogados:**

- **P2 `D-novo-WEBHOOK-PJ-SLOT-UNICO`** (A4 revisor financeiro) — o listener `cooper-token-compra-pj.listener.ts` continua usando idempotência via slot `CooperTokenCompra.ultimoWebhookEventId` (check-then-act sem UNIQUE constraint), enquanto o branch principal migrou pra WebhookEvent+P2002 nesta corretiva. Race extrema pode gerar creditação dupla de tokens de compra PJ. **Fix:** migrar listener PJ pra também consultar `WebhookEvent` OU adicionar `@@unique([provider, eventId])` no fluxo compra-PJ. Fora do escopo desta corretiva.
- **P2 `D-novo-CT3-CRON-RECONCILIACAO`** (A5 revisor financeiro) — hook CT.3 (`criarLancamentoAutomatico`/`criarLancamentoConvenioContrato`) roda em `executarPosBaixaBestEffort` com `.catch(logger.error)`. Se falha, `LancamentoCaixa` REALIZADO commita OK mas o lançamento fiscal segregado (Art. 87) fica ausente sem retry automático. Existe `@@unique([origemTipo, origemId])` mas não há cron reconciliador. **Fix:** cron periódico que detecta `Cobranca PAGO WHERE id NOT IN (SELECT origemId FROM LancamentoContabil WHERE origemTipo='COBRANCA')` e recria. Sem cron, gaps silenciosos na segregação fiscal.
- **P3 `D-novo-ASAAS-FINDFIRST-FORA-TX`** (A7 revisor financeiro) — `asaasCobranca.findFirst` em `asaas.service.ts:520-539` roda ANTES da `$transaction`; `isPaymentConfirmed` avaliado antes do lock. Sem risco financeiro (CAS do `Cobranca.updateMany` cobre — dupla defesa), mas é débito de design. **Fix:** mover `findFirst` pra dentro da tx num sprint de hardening.
- ✅ **RESOLVIDO 2026-07-20** `D-novo-WEBHOOK-ROLLBACK-INTEGRATION-TEST` (A6 revisor financeiro) — as 2 metades foram provadas contra o banco Supabase real:
  - **METADE 1 — DEDUP LIVE (script one-off `__smoke-webhook-setup.ts`, deletado após uso):** setup completo (cooperado SMOKE + uc + contrato + cobrança CLUBE + asaasCobranca) via Prisma direto; POST do MESMO eventId 2× no `/asaas/webhook` do backend rodando; 1ª resposta `{received:true}` (200), 2ª `{received:true, skipped:'duplicado'}` (200); SELECTs pós: `webhook_events`=1 linha, `cobranca.status=PAGO valorPago=100`, `cooperTokenLedger`=1 crédito de 19.6 tokens (100×20%×0.98 após taxa emissão default), `cooperTokenSaldo=19.6`. Notificação WA foi pro Luciano (27981341348 whitelist); email SKIPPED (alias `+smoke` não é whitelist canônica) → zero risco pra cooperado real. Cleanup completo.
  - **METADE 2 — ROLLBACK INTEGRATION (spec permanente `backend/src/asaas/asaas-webhook-rollback.integration.spec.ts`):** contra o banco real com mock temporário do `CobrancasService.darBaixaTx` que throw. Teste 1: throw propaga `SMOKE-ROLLBACK-INJECTION` + ZERO `webhook_events` + Cobranca continua `PENDENTE` + AsaasCobranca continua `PENDING` + `executarPosBaixaBestEffort` NÃO alcançado + log `"efeito essencial falhou — Asaas vai re-tentar"`. Teste 2: re-entrega após rollback completa fluxo normal (prova que rollback não deixou estado sujo) — `webhook_events`=1, `executarPosBaixaBestEffort` alcançado, AsaasCobranca `RECEIVED`. Cleanup preemptivo + afterAll garantido.
  - **A idempotência do webhook Asaas está PROVADA end-to-end**, não só desenhada. Regression guard permanente no repo.

- **P2 `D-novo-CT-MLM-ATOMICO`**

- **P2 `D-novo-CT-MLM-ATOMICO`** — `processarPrimeiraFaturaPaga` no `indicacoes.service.ts:281` NÃO é atômico. Sequência: flip `Indicacao.status = 'PRIMEIRA_FATURA_PAGA'` em `:311` **ANTES** de criar `BeneficioIndicacao`. Se crash no meio (queda de conexão DB, OOM, deploy no momento errado), status fica avançado sem benefício — indicador perde bônus silenciosamente e nenhum retry re-executa. **Fix futuro:** (a) envolver flip + create em `$transaction` única; (b) cron de reconciliação periódico que busca `Indicacao WHERE status='PRIMEIRA_FATURA_PAGA' AND id NOT IN (SELECT indicacaoId FROM BeneficioIndicacao)` e cria o benefício faltante. Achado durante Fase 1 da Corretiva Asaas Webhook (`cobranca.primeira.paga` event → indicacoes listener). Não fixado nesta sessão — sessão dedicada MLM.

- **🔴 P1 `D-novo-SCHEMA-DRIFT-EMAILALIASCAMPANHA` (ELEVADO 2026-07-20 — bloqueia próximo db push)** — coluna `contratos_convenio.emailAliasCampanha` existe no banco Supabase mas NÃO no `schema.prisma` do `main`. **NÃO tratar como órfão-pra-dropar** — o drift é a **pegada de sprint PAUSADA e VIVA** `feature/mascara-email-convenio` (branch existe local + `origin`, 1 commit ahead: `7745082 feat(campanha): máscara de e-mail por convênio (Blocos A-E + G piloto Santi)`). O schema do branch RE-ADICIONA `emailAliasCampanha String? @unique` em `ContratoConvenio:1808`. Auditoria read-only (2026-07-20): 1 valor não-null no banco — `"santi"` — piloto ATIVO da campanha do Santi Medicina Diagnostica (CNPJ 12.033.286/0001-90, tenant CoopereBR principal, criado 2026-06-09). **Dropar = perder configuração de campanha ativa + churn na retomada do sprint.** **Consequência ativa:** `prisma db push` bloqueia com aviso de data-loss → próxima alteração de schema trava.

  **Fix definitivo (SEM `migrate dev`, SEM `--accept-data-loss`):**
  1. **Adicionar** `emailAliasCampanha String? @unique` de volta em `main:schema.prisma:ContratoConvenio` casando com o banco. **NÃO drop.** Reforça que o campo está reservado pro sprint da máscara quando ele retomar.
  2. Rodar `prisma db push` (com PM2 parado) e **confirmar `"no changes"`** — schema==banco, drift resolvido. Nada mais a selar.

  **NÃO usar `prisma migrate dev`.** O projeto usa `db push` desde 20260526 (CLAUDE.md: *"db push em dev, NUNCA migrate"*). O banco está meses à frente da história de `migrations/` — `migrate dev` detectaria drift e proporia RESET DO BANCO ("all data will be lost") no banco de produção com 307 cooperados. É o incidente que a regra existe pra evitar.

  **Sobre a tabela `webhook_events` (criada nesta corretiva via SQL raw one-off):** JÁ está no schema.prisma E no banco. Não precisa "selar" — o projeto não usa `migrate reset`, portanto "sem migration formal" é non-issue. Rota atual `db push` continua sendo canônica.

  **⚠ Regra CLAUDE.md preservada**: NUNCA `--accept-data-loss` sem OK explícito. Este fix NÃO aciona a regra — só adição de campo casando com o banco (aumento de compat, zero data-loss).

- **P2 `D-novo-MIGRATIONS-ABANDONADAS`** (PLANEJADO, NÃO agora) — pasta `backend/prisma/migrations/` congelada em 20260526 (5 migrations reais + 14 `.sql` soltos que nem são migrations Prisma canônicas). O projeto usa `db push` desde então (política CLAUDE.md); banco drifou meses à frente da história de migrations. Consequência: `migrate reset`, `migrate deploy` num ambiente novo, ou onboarding via `migrations/` NÃO reproduz o banco atual. **Reconciliação real** exige tarefa dedicada e cuidadosa: (a) baseline do schema atual como migration inicial nova (Prisma `migrate resolve --applied` sobre um snapshot); (b) validação end-to-end contra o banco vivo; (c) migração da política oficial de `db push` pra `migrate dev`. **JAMAIS via `migrate dev` casual** — reset do banco de produção é irrecuperável. Por ora, `db push` continua sendo a rota. Sessão dedicada quando for prioridade real (ex.: onboarding de novo ambiente).

**Última atualização:** 2026-07-20 — **Corretiva CooperToken FASE 2** (unique parcial + Serializable + jti anti-replay). **1 NOVO P2 catalogado:** `D-novo-CT-QR-DIRIGIDO-A2` — QR de pagamento entre cooperados do mesmo tenant NÃO amarra `recebedorId` na assinatura JWT (só valida `cooperativaId`). Qualquer cooperado do mesmo tenant que **intercepta a string do QR** (ombro, screenshot, WhatsApp compartilhado, XSS no portal) pode redimir pra si. Risco residual: cessão P2P inadvertida entre cooperados-do-mesmo-tenant. Anti-replay via `jti` (Corretiva FASE 2) já mata o vetor pior (mesmo QR usado 2x); este débito cobre o vetor complementar (QR usado por quem não deveria). **Fix proposto (A.2 da Fase 1):** payload JWT ganha `recebedorId?` opcional. UI de gerar QR ganha modo "dirigido" (pagador informa recebedor antes de gerar) vs modo "aberto" (comportamento atual). Se `recebedorId` está no payload, `processarPagamentoQr` valida match; se ausente, aceita qualquer recebedor mesmo-tenant (compat). Refactor pequeno mas toca controller + DTO + tela — fora do escopo desta corretiva. **Origem:** decisão A.1 da Fase 1 da Corretiva CooperToken (2026-07-20).
