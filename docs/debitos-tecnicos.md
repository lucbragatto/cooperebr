...
- **🔴 P1 `D-novo-SCHEMA-DRIFT-EMAILALIASCAMPANHA` (ELEVADO 2026-07-20 — bloqueia próximo db push + migration ausente do webhook_events)** — coluna `contratos_convenio.emailAliasCampanha` existe no banco Supabase mas NÃO no `schema.prisma` do `main`. **RECOMENDAÇÃO CORRIGIDA (2026-07-20 após alerta do Luciano):** NÃO tratar como órfão-pra-dropar — o drift é a **pegada de sprint PAUSADA e VIVA** `feature/mascara-email-convenio` (branch existe local + `origin`, 1 commit ahead: `7745082 feat(campanha): máscara de e-mail por convênio (Blocos A-E + G piloto Santi)`). O schema do branch RE-ADICIONA `emailAliasCampanha String? @unique` em `ContratoConvenio:1808`. Auditoria read-only (2026-07-20): 1 valor não-null no banco — `"santi"` — piloto ATIVO da campanha do Santi Medicina Diagnostica (CNPJ 12.033.286/0001-90, tenant CoopereBR principal, criado 2026-06-09). **Dropar = perder configuração de campanha ativa + churn na retomada do sprint.** **Consequências ativas:** (a) `prisma db push` bloqueia com aviso de data-loss → próxima alteração de schema trava; (b) a tabela `webhook_events` (criada nesta corretiva via SQL raw one-off) NÃO tem migration correspondente — `prisma migrate reset` OU recriação do banco a partir de migrations NÃO recria a tabela + o UNIQUE index. Idempotência do webhook depende de infra fora do versionamento canônico. **Fix corrigido (2 subitens independentes):** (1) **Drift emailAlias**: adicionar `emailAliasCampanha String? @unique` de volta em `main:schema.prisma:ContratoConvenio` casando com o banco (**NÃO drop**) — reforça que o campo está reservado pro sprint da máscara quando ele retomar. (2) **Migration formal do webhook_events**: `prisma migrate dev --name add_webhook_events` (após resolver 1) pra selar schema↔banco e garantir que a idempotência do Asaas Webhook Fase 2 sobrevive a `migrate reset`. Descoberto na Corretiva Asaas Webhook 2026-07-20.

  **⚠ Regra CLAUDE.md preservada**: NUNCA `--accept-data-loss` sem OK explícito do Luciano. Este débito NÃO envolve `--accept-data-loss` na nova recomendação — só adição de campo casando com o banco.
