...
- **🔴 P1 `D-novo-WEBHOOK-ROLLBACK-INTEGRATION-TEST` (ELEVADO 2026-07-20 — fazer ANTES do Asaas processar dinheiro real)** (A6 revisor financeiro) — o núcleo da Fase 2 (idempotência via UNIQUE real do Postgres + rollback real da tx) está provado só com Prisma **MOCKADO**. Design está correto mas comportamento em runtime não foi verificado end-to-end. **Smoke de 5 min contra o banco rodando obrigatório antes de qualquer webhook real:** (a) **Dedup:** `POST` do MESMO evento 2× → 2ª = `{skipped:'duplicado'}` (200); conferir no banco: 1 linha em `webhook_events` + Cobrança baixada UMA vez + tokens creditados UMA vez. (b) **Rollback:** injetar falha no meio da tx (ex.: mock temporário do `darBaixaTx` throw) → confirmar ZERO linha em `webhook_events` + ZERO baixa parcial + resposta 500. Só depois disso a idempotência está PROVADA, não só desenhada. **Escopo mínimo:** 2 requests via `curl`/`invoke-restmethod` + 4 queries SELECT no banco de dev.
