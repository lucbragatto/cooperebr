...
- **P3 `D-novo-WEBHOOK-ROLLBACK-INTEGRATION-TEST`** (A6 revisor financeiro) — testes C2/C3 do `asaas-webhook-idempotencia.spec.ts` provam contrato de propagação de erro mas NÃO o rollback real do Postgres (mock não simula). **Fix:** teste de integração com banco de teste real pra confirmar rollback end-to-end.
...
- **P3 `D-novo-DRIFT-EMAILALIASCAMPANHA`** — coluna `contratos_convenio.emailAliasCampanha` existe no banco Supabase (1 valor não-nulo) mas foi removida do schema Prisma há tempos (zero referências em `backend/src`). `prisma db push` bloqueia com aviso de data-loss. Comprovado seguro por grep — código morto. **Fix futuro:** auditoria (script one-off que mostra o único valor + linha original) → decisão do Luciano → `db push --accept-data-loss` OU `ALTER TABLE contratos_convenio DROP COLUMN emailAliasCampanha` manual + `prisma generate`. Descoberto na Corretiva Asaas Webhook 2026-07-20 quando `db push` bloqueou aplicação do model `WebhookEvent` (bypass foi via SQL raw one-off).
