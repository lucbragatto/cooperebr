...
const TIPOS = [
  ...
  { valor: 'ASSOCIACAO', label: 'Associação', membro: 'Associado' },
  { valor: 'CONDOMINIO', label: 'Condomínio', membro: 'Condômino' },
];
...
/* ─── Stepper (componente extraído) ──────────────────────── */

function Stepper({ step, usinaConfigurada }: { step: number; usinaConfigurada: boolean }) {
...
/* ─── STEP 1: Dados do parceiro (componente extraído) ──── */

function Step1({
  parceiro,
  setPField,
}: {
  parceiro: { nome: string; tipoParceiro: string; cnpj: string; email: string; telefone: string; nomeResponsavel: string; cpfResponsavel: string; cidade: string; estado: string; cep: string; endereco: string };
  setPField: (field: string, value: string) => void;
}) {
...
/* ─── STEP 2: Usina (componente extraído) ──── */
...
function Step2({
  ...
  temUsina: string;
  setTemUsina: (v: 'sim' | 'nao' | '') => void;
  usinaExistenteId: string;
  setUsinaExistenteId: (v: string) => void;
  usinasDisponiveis: any[];
  novaUsina: any;
  setUField: (field: string, value: any) => void;
      ...
      {temUsina === 'sim' && (
        <div className="space-y-3 border border-gray-200 rounded-lg p-4">
          ...
          {usinasDisponiveis.length === 0 ? (
            <p className="text-sm text-gray-500">Nenhuma usina cadastrada.</p>
          ) : (
        ...
        <div className="space-y-3 border border-gray-200 rounded-lg p-4">
          ...
          {capacidadeKwh && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 text-sm text-blue-800">
              Capacidade mensal estimada: <strong>{Number(capacidadeKwh).toLocaleString('pt-BR')} kWh</strong> (kWp x 120h)
            </div>
          )}
...
/* ─── STEP 3: Lista de espera (componente extraído) ──── */
...
function Step3({
  ...
  carregandoEspera: boolean;
  listaEspera: any[];
  alocarAutomatico: boolean;
  setAlocarAutomatico: (v: boolean) => void;
}) {
  return (
    <div className="space-y-4">
...
/* ─── STEP 4: Plano SaaS (componente extraído) ──── */
...
function Step4({
  ...
  setPlanoSelecionado: (v: string | null) => void;
  diaVencimento: string;
  setDiaVencimento: (v: string) => void;
  statusSaas: string;
  setStatusSaas: (v: string) => void;
}) {
  return (
    <div className="space-y-4">
...
/* ─── STEP 5: Modelo de cobrança (componente extraído) ──── */
...
function Step5({
  ...
  setModeloCobranca: (v: string) => void;
  valorModelo: string;
  setValorModelo: (v: string) => void;
  descontoPadrao: string;
  setDescontoPadrao: (v: string) => void;
  multaAtraso: string;
  setMultaAtraso: (v: string) => void;
  jurosDiarios: string;
  setJurosDiarios: (v: string) => void;
}) {
...
export default function NovaCooperativaPage() {
  ...
  function setPField(field: string, value: string) {
    setParceiro((p) => ({ ...p, [field]: value }));
  }

  function setUField(field: string, value: any) {
  ...
  useEffect(() => {
    if (step === 2 && usinasDisponiveis.length === 0) {
      api.get('/usinas').then(({ data }) => {
        setUsinasDisponiveis(Array.isArray(data) ? data : []);
      }).catch(() => {});
    }
  }, [step]);
    ...
    <div>
      ...
      <Card className="max-w-3xl">
        <CardContent className="pt-6">
          <Stepper step={step} usinaConfigurada={usinaConfigurada} />

          {step === 1 && <Step1 parceiro={parceiro} setPField={setPField} />}
          {step === 2 && (
            <Step2
              temUsina={temUsina}
              setTemUsina={setTemUsina}
              usinaExistenteId={usinaExistenteId}
              setUsinaExistenteId={setUsinaExistenteId}
          ...
          {step === 3 && (
            <Step3
              carregandoEspera={carregandoEspera}
              listaEspera={listaEspera}
              alocarAutomatico={alocarAutomatico}
              setAlocarAutomatico={setAlocarAutomatico}
            />
          )}
          {step === 4 && (
            <Step4
              ...
              statusSaas={statusSaas}
              setStatusSaas={setStatusSaas}
            />
          )}
          {step === 5 && (
            <Step5
              modeloCobranca={modeloCobranca}
              setModeloCobranca={setModeloCobranca}
              valorModelo={valorModelo}
              setValorModelo={setValorModelo}
              descontoPadrao={descontoPadrao}
              setDescontoPadrao={setDescontoPadrao}
              multaAtraso={multaAtraso}
              setMultaAtraso={setMultaAtraso}
              jurosDiarios={jurosDiarios}
              setJurosDiarios={setJurosDiarios}
            />
          )}
