...
const TIPOS = [
  { value: 'COOPERATIVA', label: 'Cooperativa', membro: 'Cooperados' },
  { value: 'CONSORCIO', label: 'Consórcio', membro: 'Consorciados' },
  { value: 'ASSOCIACAO', label: 'Associação', membro: 'Associados' },
  { value: 'CONDOMINIO', label: 'Condomínio', membro: 'Condôminos' },
];
...
export default function Step1Dados({ defaultValues, onSubmit }: Step1Props) {
  const [form, setForm] = useState({
    nome: defaultValues?.nome || '',
    tipoParceiro: defaultValues?.tipoParceiro || 'COOPERATIVA',
    cnpj: defaultValues?.cnpj || '',
    email: defaultValues?.email || '',
    telefone: defaultValues?.telefone || '',
    nomeResponsavel: defaultValues?.nomeResponsavel || '',
    cpfResponsavel: defaultValues?.cpfResponsavel || '',
  ...
  const tipoInfo = TIPOS.find((t) => t.value === form.tipoParceiro);

  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-neutral-800">Dados do Parceiro</h2>
      ...
      <div>
        ...
        <select className={cls} value={form.tipoParceiro} onChange={(e) => set('tipoParceiro', e.target.value)}>
          {TIPOS.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
        </select>
        {tipoInfo && (
          <p className="text-xs text-green-600 mt-1">Membros serão chamados de: <strong>{tipoInfo.membro}</strong></p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          {erros.nomeResponsavel && <p className="text-xs text-red-500 mt-0.5">{erros.nomeResponsavel}</p>}
        </div>
      </div>

      <div>
        <label className={lbl}>CPF do responsável</label>
        <input className={cls} value={form.cpfResponsavel} onChange={(e) => set('cpfResponsavel', maskCpf(e.target.value))} placeholder="000.000.000-00" />
      </div>
