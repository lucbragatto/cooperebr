...
export default function ParceiroConfiguracoesPage() {
  ...
  // Sprint Clube P1 — Fase 1.5 Bloco 4 (10/06/2026): a edicao da
  // ConfigCooperToken foi MOVIDA pra /dashboard/cooper-token/config (pagina
  // dedicada). Esta pagina mantem so um LINK pra la — sem state/handler
  // duplicado.
  ...
  useEffect(() => {
    async function carregar() {
      try {
        ...
        if (me.cooperativaId) {
          const coopRes = await api.get(`/cooperativas/${me.cooperativaId}`);
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      {/* Sprint Clube P1 — Fase 1.5 Bloco 4 (10/06/2026): CooperToken vira
          link pra pagina dedicada /dashboard/cooper-token/config. Antes
          esta secao duplicava 8 campos editaveis; agora a config completa
          (8 antigos + 8 taxas + 3 oxidacao = 19 campos) vive numa pagina
          so — fonte unica. */}
      <Card>
        <CardHeader>
          <CardTitle className="text-sm font-medium flex items-center gap-2">
        ...
        <CardContent className="space-y-3">
          <p className="text-sm text-gray-700">
            A configuração da economia do CooperToken — modo de geração, vida do
            token, valor em R$, taxas de operação (emissão, QR, transferência,
            resgate) e oxidação — vive numa página dedicada dentro do Clube.
          </p>
          <Link
