...
import {
  ArrowDownCircle, Calendar, CheckCircle, Clock, AlertTriangle, Plus, Search, Loader2,
} from 'lucide-react';
...
export default function ContasReceberPage() {
  ...
  const [form, setForm] = useState({
    descricao: '', valor: '', dataVencimento: '', competencia: competenciaAtual(),
    cooperadoNome: '', planoContasId: '',
  });
  ...
  async function criarLancamento() {
    ...
    try {
      ...
      setForm({ descricao: '', valor: '', dataVencimento: '', competencia: competenciaAtual(), cooperadoNome: '', planoContasId: '' });
      carregar();
    } catch {
  ...
  const filtrados = lancamentos.filter((l) =>
    !busca || (l.descricao?.toLowerCase().includes(busca.toLowerCase())) || (l.cooperado?.toLowerCase().includes(busca.toLowerCase()))
  );

  const isVencido = (l: Lancamento) => l.status === 'PREVISTO' && l.dataVencimento && l.dataVencimento < hoje;

  const totalReceber = filtrados.filter((l) => l.status === 'PREVISTO').reduce((s, l) => s + Number(l.valor), 0);
  const vencendoHoje = filtrados.filter((l) => l.status === 'PREVISTO' && l.dataVencimento?.slice(0, 10) === hoje).length;
  const vencidos = filtrados.filter((l) => isVencido(l)).length;
  const recebidoMes = filtrados.filter((l) => l.status === 'REALIZADO').reduce((s, l) => s + Number(l.valor), 0);
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            type="text"
            placeholder="Buscar por cooperado ou descrição..."
            value={busca}
                  ...
                  filtrados.map((l) => {
                    ...
                    return (
                      <TableRow key={l.id} className={vencido ? 'bg-red-50' : ''}>
                        <TableCell className="font-medium max-w-[240px] truncate">{l.descricao}</TableCell>
                        ...
                        <TableCell>{l.cooperado || '—'}</TableCell>
                        <TableCell>
                          <Badge variant="outline" className={badge.class}>{badge.label}</Badge>
                        </TableCell>
                        <TableCell>
                          {l.status === 'PREVISTO' && (
                            <Button size="sm" variant="outline" onClick={() => realizar(l.id)}>
                              <CheckCircle className="h-3.5 w-3.5 mr-1" /> Realizar
                            </Button>
                          )}
                        </TableCell>
                      </TableRow>
      ...
      <Dialog open={modalAberto} onOpenChange={setModalAberto}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Novo Lançamento — Receita</DialogTitle>
          </DialogHeader>
