...
export default function AgregadoresPage() {
  const [agregadores, setAgregadores] = useState<Agregador[]>([]);
  const [carregando, setCarregando] = useState(true);
  const [busca, setBusca] = useState('');

  function carregar() {
    api.get<Agregador[]>('/administradoras')
      .then(r => setAgregadores(r.data))
      .finally(() => setCarregando(false));
  }

  useEffect(() => { carregar(); }, []);

  const filtrados = agregadores.filter(a => {
        ...
        <CardHeader>
          <div className="flex items-center justify-between gap-4">
            <CardTitle className="text-base font-medium text-gray-600">
              {carregando ? 'Carregando...' : `${filtrados.length} agregadores`}
            </CardTitle>
            ...
            <TableBody>
              {carregando ? (
                ...
                filtrados.map(a => (
                  <TableRow key={a.id}>
                    <TableCell>
                      <Link href={`/parceiro/agregadores/${a.id}`} className="font-medium text-blue-600 hover:underline">{a.razaoSocial}</Link>
